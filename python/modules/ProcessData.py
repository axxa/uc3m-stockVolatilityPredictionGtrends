...
def calculate_value_variation(data_array: np.array):
    arr = np.zeros_like(data_array)
    for idx in range(data_array.shape[0]):  # or range(len(theta))
        if idx != 0:
            arr[idx] = (data_array[idx - 1] / data_array[idx])
        else:
            arr[idx] = 0
    return arr
...
class ProcessData:
    ...
    def generate_binary_series(self, data_array: np.array, column_name, discard_first):
